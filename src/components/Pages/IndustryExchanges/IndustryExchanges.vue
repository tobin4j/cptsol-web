...
export default {
  name: 'CenterDynamics',
   ...
   methods: {
    ...
    goDetails(id) {
      ...
      let noticeUrl=`https://api.cptsol.cn/api/open/articleDetail?type=4&page=${this.pageNum}&size=10&id=${id}`;
    ...
    getDataList() {
      let noticeUrl=`https://api.cptsol.cn/api/open/articleList?type=4&page=${this.pageNum}&size=10&title=${this.keyWord}`;
      axios.get(noticeUrl).then((res)=>{
  ...
  setup() {
    ...
    onMounted(async () => {
      var noticeUrl="https://api.cptsol.cn/api/open/articleList?type=4&page=1&size=10";
      (async function () {
